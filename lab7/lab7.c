...
void groupByKey(IntermediateInput *input, Output *output, int *result_count) {
  ...
  while (i < *result_count) {
    if (output[i].doubled_value == input->doubled_value) {
      // Found existing group, add line number
      output[i].line_numbers[output[i].count] = input->line_number;
      output[i].count++;
      return; // Found, so we are done.
    }
    i++;
  }

  // If loop finishes, no group was found. Add a new one.
  ...
  (*result_count)++; // Increment the total number of unique groups
}
...
void reduce(Output *output) {
  ...
  // Use a flag to control comma printing
  int is_first = 1;
